...
var Datas = (function () {
	...
	Datas.prototype.comDocumento = function (_documento, locate = 'usa') {
		...
		if (!validarData(_documento)) {
			throw new Error('O ano do documento deve ser maior que 1997');
		}
...
var Boleto = (function () {
	...
	let NOVA_DATA_BASE = new Date(2025, 2 - 1, 22);
	...
	Boleto.prototype.getFatorVencimento = function () {
		...
		const usarNovaDataBase = vencimento >= NOVA_DATA_BASE;
		const dataBase = usarNovaDataBase ? NOVA_DATA_BASE : DATA_BASE;
		const fatorVencimento = usarNovaDataBase ? 1000 : 0;
		
		const diferencaEmDias = (vencimento - dataBase) / (1000 * 60 * 60 * 24);
		
		console.log(vencimento, NOVA_DATA_BASE, usarNovaDataBase, fatorVencimento, diferencaEmDias);
		if (diferencaEmDias > 10000) {
			throw new Error('Data fora do formato aceito');
		}

		return Math.floor(fatorVencimento + diferencaEmDias).toString();
	};
